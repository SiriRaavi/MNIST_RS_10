...
def runOnce():
    ...
    ten_items_arr = fm.SelectItemsFromDir(src_dir,2)
...
def runEveryTrainIter(i):
    ...
    ten_items_arr = fm.SelectItemsFromDir(dst_dir_itm, 2)
...
def runSimpleValidCopy():
    ten_items_arr = fm.SelectItemsFromDir(src_val_dir, 2)
    ...
    fm.MakeDir(dst_dir0)
    fm.MakeDir(dst_dir1)
    path0 = sample_train_1_paths[0]
    path1 = sample_train_1_paths[1]
    for each in path0:
        fm.Copy(each,dst_dir0)
    for each in path1:
        fm.Copy(each,dst_dir1)
